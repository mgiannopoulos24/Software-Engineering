...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  // <-- ΑΛΛΑΓΗ: Το prop είναι πλέον ένα ενιαίο αντικείμενο ZoneDataWithType
...
export const ZoneManagementDialog: React.FC<Props> = ({ isOpen, onClose, zone, onSave, onDelete }) => {
  ...
  useEffect(() => {
    if (zone && isOpen) {
      ...
      // Ελέγχουμε τον τύπο απευθείας από το αντικείμενο zone
  ...
  const handleSave = async () => {
    ...
    const saveData: ZoneDataWithType = zone.type === 'interest'
      ? { ...zone, name, radiusInMeters: radius, constraints }
      : { ...zone, name, radiusInMeters: radius };

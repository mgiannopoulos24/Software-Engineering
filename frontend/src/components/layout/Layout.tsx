...
const Tooltip: React.FC<{ children: React.ReactNode; text: string }> = ({ children, text }) => (
  <div className="group relative flex items-center">
    {children}
    <div className="absolute left-1/2 top-full z-50 mt-2 hidden w-max -translate-x-1/2 transform rounded-md bg-slate-900 px-2 py-1 text-xs text-white group-hover:block">
      {text}
    </div>
  </div>
);
...
const NotificationsDropdown: React.FC = () => {
  ...
  const NotificationIcon = ({ type }: { type: AppNotification['type'] }) => {
    ...
    if (type === 'violation')
      return <CircleAlert className="h-5 w-5 flex-shrink-0 text-yellow-500" />;
    ...
    <div className="relative" ref={dropdownRef}>
      <Tooltip text="Notifications">
        <button
          ...
          className="relative rounded-full p-2 text-slate-300 transition-colors hover:bg-slate-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-slate-800"
        >
          <Bell size={20} />
          {unreadCount > 0 && (
            <span className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-xs font-bold text-white ring-2 ring-slate-800">
          ...
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((notif) => (
                <div
                  key={notif.id}
                  className="flex items-start gap-3 border-b p-4 last:border-b-0 hover:bg-slate-50"
                >
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="hidden items-center space-x-2 md:flex">
              {isAuthenticated ? (
                ...
                <div className="flex space-x-2">
                  <Link
                    to="/login"
                    className="inline-flex items-center gap-2 rounded-md border border-transparent px-4 py-2 text-sm font-medium transition-all duration-200 hover:border hover:border-slate-400"
                  >
                    <LogIn size={18} />
                    <span>Login</span>
                  </Link>
                  ...
                  {isUserDropdownOpen && (
                    <div className="absolute right-0 z-[1000] mt-2 w-56 origin-top-right rounded-md bg-white py-1 text-slate-800 shadow-lg ring-1 ring-black ring-opacity-5">
                      <div className="px-4 py-2 text-sm text-slate-500">
                        <p className="truncate font-medium text-slate-800">{currentUser?.email}</p>
            ...
            <div className="flex items-center gap-2 md:hidden">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-md p-2 text-slate-300 hover:bg-slate-700"
              >

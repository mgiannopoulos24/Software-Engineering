import {
    getInterestZone, createOrUpdateInterestZone, deleteInterestZone,
    getCollisionZone, createOrUpdateCollisionZone, deleteCollisionZone
...
export const ZoneProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const loadZones = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // Φορτώνουμε και τις δύο ζώνες παράλληλα
    ...
    // Φόρτωση των ζωνών όταν αλλάζει ο χρήστης
    useEffect(() => {
        void loadZones();
    }, [loadZones]);
    ...
    const saveInterestZone = async (zoneData: ZoneOfInterestDTO) => {
        try {
            ...
            throw error; // Το ρίχνουμε ξανά για να το πιάσει το UI αν χρειαστεί
        }
    };

...
export const createOrUpdateInterestZone = async (
  zoneData: ZoneOfInterestDTO
): Promise<ZoneOfInterestDTO> => {
  const token = getToken();
  if (!token) throw new Error('Authentication token not found.');
...
export const createOrUpdateCollisionZone = async (
  zoneData: CollisionZoneDTO
): Promise<CollisionZoneDTO> => {
  const token = getToken();
  if (!token) throw new Error('Authentication token not found.');
